...
import { ArrowRight, Vote, Clock, Users, Shield, Zap, Lock, Globe, Eye, Bot, Smartphone, Accessibility, BarChart3, Languages, Check, Star } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#0b0c0f] text-white overflow-x-hidden">
      ...
      {liveElections.length > 0 && (
        <section className="max-w-5xl mx-auto px-6 pb-24">
          <div className="flex items-center justify-between mb-8">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.15em] text-[#4fffb0] mb-2">Live Now</p>
              <h2 className="text-3xl font-extrabold">Active Elections</h2>
            </div>
          </div>
              ...
              <div key={election._id} className="bg-[rgba(255,255,255,0.03)] border border-[rgba(79,255,176,0.15)] rounded-2xl p-6 hover:border-[rgba(79,255,176,0.3)] transition-all">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-[rgba(79,255,176,0.1)] flex items-center justify-center"><Vote className="w-5 h-5 text-[#4fffb0]" /></div>
                    <div>
                      <h3 className="font-bold text-white">{election.title}</h3>
                      <p className="text-xs text-[rgba(255,255,255,0.4)] mt-1">Ends {new Date(election.endDate).toLocaleDateString()}</p>
                    </div>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4 text-xs text-[rgba(255,255,255,0.3)]">
                    ...
                    <span className="flex items-center gap-1"><Vote className="w-3 h-3" /> {election.totalVotes || 0} votes</span>
                  </div>
                  <Link href={`/vote?election=${election._id}`} className="px-4 py-2 rounded-lg bg-[#4fffb0] text-[#0b0c0f] text-xs font-bold hover:opacity-90 transition-all flex items-center gap-1">Vote Now <ArrowRight className="w-3 h-3" /></Link>
                </div>
              </div>
            ))}
          </div>
      ...
      <section id="features" className="max-w-5xl mx-auto px-6 py-24">
        <div className="max-w-xl mb-16">
          ...
          <h2 className="text-3xl md:text-4xl font-extrabold leading-tight">Security that <span className="text-[rgba(255,255,255,0.5)] italic">scales</span> with democracy.</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { title: "Zero-Knowledge Encryption", desc: "Military-grade AES-256. Votes are cryptographically sealed." },
            { title: "Blockchain Immutable", desc: "Every vote hashed on-chain. Tamper-proof by mathematical certainty." },
            { title: "Real-Time Streaming", desc: "WebSocket-powered live results with sub-100ms latency." },
            { title: "AI Fraud Detection", desc: "Deep learning models flag anomalies in real-time." },
            { title: "End-to-End Verifiable", desc: "Voters independently verify their ballot was counted." },
            { title: "Global Edge Network", desc: "300+ edge locations for instant load times worldwide." },
          ].map((f,i) => (
      ...
      <section id="pricing" className="max-w-5xl mx-auto px-6 py-24">
        <div className="text-center mb-16">
          ...
          <h2 className="text-3xl md:text-4xl font-extrabold">Simple, <span className="text-[rgba(255,255,255,0.5)] italic">honest</span> pricing.</h2>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { plan: "Starter", price: "Free", features: ["500 voters","5 elections/mo","Basic analytics","Email support"], btn: "Start free", featured: false },
            { plan: "Professional", price: "$299/mo", features: ["10,000 voters","Unlimited elections","Advanced analytics","Priority support","API access","AI detection"], btn: "Get started", featured: true },
            { plan: "Enterprise", price: "Custom", features: ["Unlimited voters","Dedicated infra","99.99% SLA","24/7 support","On-premise","Security audit"], btn: "Contact sales", featured: false },
          ].map((p,i) => (
            <div key={i} className={"bg-[rgba(255,255,255,0.03)] border rounded-2xl p-8 relative " + (p.featured ? "border-[rgba(79,255,176,0.2)] bg-[rgba(79,255,176,0.04)]" : "border-[rgba(255,255,255,0.07)]")}>
              {p.featured && <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 rounded-full text-[10px] font-bold bg-[#4fffb0] text-[#0b0c0f] uppercase">Most Popular</div>}
              ...
              <ul className="space-y-2 mb-8">{p.features.map((f,j) => <li key={j} className="text-sm text-[rgba(255,255,255,0.6)]">✓ {f}</li>)}</ul>
              <Link href="/register" className={"block w-full py-3 rounded-xl text-center text-sm font-semibold transition-all " + (p.featured ? "bg-[#4fffb0] text-[#0b0c0f] hover:opacity-90" : "border border-[rgba(255,255,255,0.1)] text-[rgba(255,255,255,0.6)] hover:text-white")}>{p.btn}</Link>
            </div>
          ))}
        </div>
      </section>
      ...
      <div className="border-t border-[rgba(255,255,255,0.07)] py-24 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            ...
            <h2 className="text-3xl font-extrabold">What our <span className="text-[rgba(255,255,255,0.5)] italic">customers say.</span></h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <div key={i} className="bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.07)] rounded-2xl p-8">
                <div className="text-[#ffb347] text-sm mb-4">★★★★★</div>
                <p className="text-sm text-[rgba(255,255,255,0.6)] leading-relaxed mb-6 italic">"{t.quote}"</p>
                <div className="flex items-center gap-3">
                  ...
                  <div><div className="text-sm font-bold">{t.name}</div><div className="text-xs text-[rgba(255,255,255,0.3)]">{t.role}</div></div>
                </div>
              </div>
            ))}
          </div>
