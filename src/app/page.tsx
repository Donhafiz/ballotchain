        ...
        <section className="relative isolate min-h-screen overflow-hidden border-b border-white/5">
          ...
          <div className="mx-auto grid min-h-screen max-w-7xl grid-cols-1 items-center gap-20 px-6 py-32 lg:grid-cols-[1.15fr_0.85fr] lg:px-10">
            <motion.div initial="hidden" animate="visible" variants={fadeUp} className="relative z-10">
              ...
              <motion.h1 custom={2} variants={fadeUp} className="mt-10 max-w-5xl text-6xl font-black leading-[0.88] tracking-[-0.07em] sm:text-7xl xl:text-[110px]">
                The Future
                <span className="block bg-gradient-to-r from-emerald-300 via-cyan-300 to-violet-300 bg-clip-text font-serif italic text-transparent">of Digital Democracy</span>
                Starts Here.
              </motion.h1>
              <motion.p custom={3} variants={fadeUp} className="mt-8 max-w-2xl text-lg leading-9 text-white/50">
                BallotChain is a next-generation election infrastructure platform combining blockchain verification, AI-powered fraud prevention, enterprise security, and real-time transparency at global scale.
              </motion.p>
              <motion.div custom={4} variants={fadeUp} className="mt-12 flex flex-col gap-5 sm:flex-row">
                <Link href="/register" className="group relative inline-flex items-center justify-center overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-300 via-cyan-300 to-violet-300 p-[1px]">
                  <div className="flex items-center gap-2 rounded-2xl bg-black px-8 py-4 text-sm font-black text-white transition-all duration-300 group-hover:bg-transparent group-hover:text-black">Launch Election <ArrowRight className="h-4 w-4 transition-transform duration-300 group-hover:translate-x-1" /></div>
                </Link>
                <Link href="/ussd-demo" className="group inline-flex items-center justify-center gap-3 rounded-2xl border border-white/10 bg-white/[0.03] px-8 py-4 text-sm font-semibold text-white backdrop-blur-2xl transition-all duration-300 hover:border-white/20 hover:bg-white/[0.05]">
                  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10"><Play className="h-4 w-4 fill-white" /></div>
                  Watch Platform Demo
                </Link>
              </motion.div>
              <motion.div custom={5} variants={fadeUp} className="mt-16 grid grid-cols-2 gap-5 md:grid-cols-4">
                {[{ value: "2.8M+", label: "Votes Secured", icon: ShieldCheck },{ value: "15K+", label: "Elections", icon: Vote },{ value: "127", label: "Countries", icon: Globe2 },{ value: "99.99%", label: "Uptime", icon: Zap }].map((stat, index) => (
                  <motion.div whileHover={{ y: -6 }} key={index} className="group relative overflow-hidden rounded-3xl border border-white/6 bg-white/[0.03] p-5 backdrop-blur-2xl">
                    <div className="absolute inset-0 bg-gradient-to-br from-emerald-400/[0.08] to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
                    <div className="relative z-10"><div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-400/10 text-emerald-300"><stat.icon className="h-5 w-5" /></div><div className="text-3xl font-black tracking-tight">{stat.value}</div><p className="mt-2 text-[11px] uppercase tracking-[0.2em] text-white/35">{stat.label}</p></div>
                  </motion.div>
                ))}
              </motion.div>
            </motion.div>
        ...
        <section id="features" className="relative px-6 py-32 lg:px-10">
          <div className="mx-auto max-w-7xl">
            <div className="text-center">
              <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] px-5 py-3 backdrop-blur-2xl"><Sparkles className="h-4 w-4 text-emerald-300" /><span className="text-[11px] font-black uppercase tracking-[0.25em] text-white/60">Enterprise Infrastructure</span></div>
              <h2 className="mx-auto mt-8 max-w-5xl text-5xl font-black leading-[1] tracking-[-0.06em] md:text-7xl">Built Like a<span className="block bg-gradient-to-r from-emerald-300 via-cyan-300 to-violet-300 bg-clip-text font-serif italic text-transparent">Modern Technology Company</span></h2>
            </div>
            <div className="mt-20 grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
              {features.map((feature, index) => (
                <motion.div key={index} whileHover={{ y: -8 }} className="group relative overflow-hidden rounded-[32px] border border-white/6 bg-white/[0.03] p-8 backdrop-blur-2xl">
                  <div className="absolute inset-0 bg-gradient-to-br from-emerald-400/[0.08] via-transparent to-cyan-400/[0.05] opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
                  <div className="relative z-10"><div className="flex h-16 w-16 items-center justify-center rounded-3xl bg-gradient-to-br from-emerald-300/15 to-cyan-300/10 text-emerald-300"><feature.icon className="h-7 w-7" /></div><h3 className="mt-8 text-2xl font-black tracking-tight">{feature.title}</h3><p className="mt-5 text-sm leading-8 text-white/50">{feature.description}</p></div>
                </motion.div>
              ))}
            </div>
          </div>
        ...
        <section className="px-6 pb-20 lg:px-10">
          <div className="mx-auto max-w-6xl">
            <div className="text-center mb-16">
              <h2 className="text-4xl font-black tracking-tight">Trust & <span className="font-serif italic text-white/50">Verify</span></h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Link href="/verify" className="group relative overflow-hidden rounded-3xl border border-white/6 bg-white/[0.03] p-8 backdrop-blur-2xl hover:border-emerald-300/20 transition-all no-underline">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-emerald-400/10 text-emerald-300 mb-6"><ShieldCheck className="h-7 w-7" /></div>
                <h3 className="text-xl font-black mb-2 text-white">Verify Your Vote</h3>
                <p className="text-sm text-white/40 mb-4">Enter your receipt to independently verify your ballot on the blockchain.</p>
                <span className="text-emerald-300 text-sm font-bold flex items-center gap-1">Verify Now <ArrowRight className="h-4 w-4" /></span>
              </Link>
              <Link href="/audit" className="group relative overflow-hidden rounded-3xl border border-white/6 bg-white/[0.03] p-8 backdrop-blur-2xl hover:border-cyan-300/20 transition-all no-underline">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-cyan-400/10 text-cyan-300 mb-6"><Eye className="h-7 w-7" /></div>
                <h3 className="text-xl font-black mb-2 text-white">Public Audit</h3>
                <p className="text-sm text-white/40 mb-4">Watch every vote appear in real-time on the transparent public dashboard.</p>
                <span className="text-cyan-300 text-sm font-bold flex items-center gap-1">View Live Feed <ArrowRight className="h-4 w-4" /></span>
              </Link>
              <Link href="/docs" className="group relative overflow-hidden rounded-3xl border border-white/6 bg-white/[0.03] p-8 backdrop-blur-2xl hover:border-violet-300/20 transition-all no-underline">
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-violet-400/10 text-violet-300 mb-6"><Globe2 className="h-7 w-7" /></div>
                <h3 className="text-xl font-black mb-2 text-white">Documentation</h3>
                <p className="text-sm text-white/40 mb-4">API references, SDKs, and integration guides for developers.</p>
                <span className="text-violet-300 text-sm font-bold flex items-center gap-1">Learn More <ArrowRight className="h-4 w-4" /></span>
              </Link>
            </div>
          </div>
        </section>
        ...
        <section className="px-6 pb-32 lg:px-10">
          <div className="mx-auto max-w-6xl overflow-hidden rounded-[42px] border border-white/10 bg-gradient-to-br from-white/[0.05] to-white/[0.02] p-10 shadow-[0_30px_140px_rgba(0,0,0,0.45)] backdrop-blur-3xl md:p-16">
            <div className="mx-auto max-w-4xl text-center">
              <div className="mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-gradient-to-br from-emerald-300/15 to-cyan-300/10 text-emerald-300"><Sparkles className="h-12 w-12" /></div>
              <h2 className="mt-10 text-5xl font-black leading-[1] tracking-[-0.06em] md:text-7xl">Redefining Trust<span className="block font-serif italic text-white/55">Through Technology.</span></h2>
              <p className="mx-auto mt-8 max-w-2xl text-lg leading-9 text-white/50">Launch elections with the same level of sophistication, security, and scalability used by world-class technology platforms.</p>
              <div className="mt-12 flex flex-col justify-center gap-5 sm:flex-row">
                ...
                <Link href="/contact" className="inline-flex items-center justify-center rounded-2xl border border-white/10 bg-white/[0.03] px-9 py-5 text-sm font-bold text-white transition-all duration-300 hover:border-white/20 hover:bg-white/[0.05]">Contact Enterprise Team</Link>
              </div>
            </div>
          </div>
        </section>
