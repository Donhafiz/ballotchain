...
import { Key, ArrowRight, Shield, Smartphone, Vote } from "lucide-react";
...
export default function VoterLoginPage() {
  ...
  const handleAccessCode = (e: React.FormEvent) => {
    ...
    window.location.href = `/voter/elections/vote?id=${targetId}&code=${accessCode}`;
  };
        ...
        <div className="w-full max-w-[480px]">
          ...
          <div className="bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.06)] rounded-2xl p-8 backdrop-blur-xl">
            ...
            <form onSubmit={handleAccessCode} className="flex flex-col gap-5">
              <div><label className="block text-xs font-bold text-[rgba(255,255,255,0.3)] uppercase mb-3">Access Code</label><input value={accessCode} onChange={(e) => setAccessCode(e.target.value.toUpperCase())} placeholder="Type DEMO or any code" className="w-full px-5 py-4 rounded-xl bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.08)] text-white text-lg font-mono tracking-[0.1em] text-center outline-none focus:border-[rgba(79,255,176,0.3)] transition-all" required /></div>
              <button type="submit" disabled={loading} className="w-full py-4 rounded-xl bg-gradient-to-r from-[#4fffb0] to-[#00d4ff] text-[#0b0c0f] text-sm font-bold hover:opacity-90 transition-all disabled:opacity-50 flex items-center justify-center gap-2">Access Ballot <ArrowRight className="w-4 h-4" /></button>
            </form>
            <div className="mt-6 p-4 rounded-xl bg-[rgba(79,255,176,0.03)] border border-[rgba(79,255,176,0.06)]"><p className="text-xs text-[rgba(255,255,255,0.3)] text-center"><Shield className="w-3 h-3 inline mr-1 text-[#4fffb0]" /> Your vote is encrypted and recorded on the blockchain</p></div>
          </div>
        </div>
      </div>
      <Footer />
