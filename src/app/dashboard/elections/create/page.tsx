...
import { Plus, Trash2, Calendar, Clock, Globe, ArrowLeft, Save, Zap, CheckCircle2, Mail } from "lucide-react";
  ...
  const handleSubmit = async () => {
    ...
    try {
      await electionsApi.create({
        ...form,
        eligibleVoters: form.voterEmails.length,
        totalVotes: 0,
        timezone: "UTC",
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex items-center gap-2">
        {[1, 2, 3].map((s) => (
          <button key={s} onClick={() => setStep(s)} className={"px-4 py-2 rounded-xl text-[12px] font-semibold transition-all " + (step === s ? "bg-[rgba(79,255,176,0.1)] text-[#4fffb0]" : "text-[rgba(255,255,255,0.3)]")}>Step {s}</button>
        ))}
      </div>
      ...
      {step === 1 && (
        <div className="bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.06)] rounded-2xl p-8 space-y-5">
          <div>
            <label className="block text-[11px] font-bold text-[rgba(255,255,255,0.3)] uppercase mb-3">Title</label>
          ...
          <div>
            <label className="block text-[11px] font-bold text-[rgba(255,255,255,0.3)] uppercase mb-3">Description</label>
            <textarea value={form.description} onChange={e => setForm({...form, description: e.target.value})} placeholder="Describe the election..." rows={3} className="w-full px-5 py-[14px] rounded-xl bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.08)] text-white text-[14px] outline-none focus:border-[rgba(79,255,176,0.3)] transition-all resize-none" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-[11px] font-bold text-[rgba(255,255,255,0.3)] uppercase mb-3"><Calendar className="w-3.5 h-3.5 inline mr-1" /> Start</label>
            ...
            <div>
              <label className="block text-[11px] font-bold text-[rgba(255,255,255,0.3)] uppercase mb-3"><Clock className="w-3.5 h-3.5 inline mr-1" /> End</label>
      ...
      {step === 3 && (
        <div className="bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.06)] rounded-2xl p-8 space-y-5">
          <div>
            <label className="block text-[11px] font-bold text-[rgba(255,255,255,0.3)] uppercase mb-3">Bulk Add Voter Emails</label>
            <textarea value={bulkEmailInput} onChange={e => setBulkEmailInput(e.target.value)} placeholder="alice@edu.com&#10;bob@edu.com&#10;carol@edu.com" rows={5} className="w-full px-5 py-[14px] rounded-xl bg-[rgba(255,255,255,0.03)] border border-[rgba(255,255,255,0.08)] text-white text-[13px] outline-none focus:border-[rgba(79,255,176,0.3)] transition-all resize-none font-mono" />
            <button onClick={addBulkEmails} className="mt-3 flex items-center gap-2 px-4 py-[9px] rounded-xl bg-[rgba(79,255,176,0.08)] text-[#4fffb0] text-[12px] font-semibold hover:bg-[rgba(79,255,176,0.12)] transition-all"><Plus className="w-3.5 h-3.5" /> Add Emails</button>
          </div>
          {form.voterEmails.length > 0 && (
            <div>
              <label className="block text-[11px] font-bold text-[rgba(255,255,255,0.3)] uppercase mb-3">Added Voters ({form.voterEmails.length})</label>
              <div className="max-h-[200px] overflow-y-auto space-y-1">
                {form.voterEmails.map((email, i) => (
                  <div key={i} className="flex items-center justify-between px-4 py-2 rounded-lg bg-[rgba(255,255,255,0.02)]">
                    ...
                    <button onClick={() => setForm({...form, voterEmails: form.voterEmails.filter((_, j) => j !== i)})} className="text-[rgba(255,255,255,0.15)] hover:text-[#EF4444]"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                ))}
              </div>
            </div>
