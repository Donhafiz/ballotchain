...
export default function PrintBallotPage() {
    ...
    <div className="min-h-screen bg-white text-black p-8 print:p-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="space-y-4 mb-8">
          ...
          <p>Mark ONE box next to your chosen candidate. Use blue or black ink.</p>
        </div>
          ...
          {candidates.map((c, i) => (
            <div key={i} className="flex items-center gap-4 p-4 border-2 border-black rounded">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600">{c.position}</div>
              </div>
            </div>
          ))}
        </div>

        <div className="mt-8 pt-4 border-t-2 border-black text-center text-sm text-gray-600">
          <p>BallotChain · Blockchain-Verified Election</p>
          <p>ballotchain.io/verify</p>
        </div>

        <button onClick={() => window.print()} className="mt-8 w-full py-3 bg-black text-white font-bold rounded print:hidden flex items-center justify-center gap-2">
          <Printer className="w-4 h-4" /> Print Ballot
        </button>
      </div>
    </div>
  );
