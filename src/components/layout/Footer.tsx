...
const footerColumns = [
  {
    title: "Platform",
    links: ["Elections", "Results", "Voters", "Analytics", "API"],
  },
  {
    title: "Company",
      ...
      <div className="max-w-[1100px] mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-[2fr_1fr_1fr_1fr] gap-8 mb-12">
          <div>
            ...
            <p className="text-[13px] text-[rgba(255,255,255,0.22)] leading-[1.7] max-w-[280px]">The world&apos;s most advanced voting platform. Enterprise-grade security, blockchain verification, and AI-powered fraud detection — trusted in 127 countries.</p>
          </div>
            ...
            <div key={i}>
              ...
              <ul className="list-none flex flex-col gap-2">
                {col.links.map((link, j) => (
                  <li key={j}>
                    <Link href={`/${link.toLowerCase()}`} className="text-[rgba(255,255,255,0.45)] text-[13px] no-underline hover:text-white transition-colors">{link}</Link>
                  </li>
                ))}
              </ul>
            </div>
